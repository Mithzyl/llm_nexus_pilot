...
**文档状态：** 进行中（第一版前端工程与核心对话外壳已实现；阶段5 `model_only_v1` 的八个接口、取消、最多两个工作 Agent 并行和费用预留前端适配已实现，真实 Agent API 端到端验收待完成）
...
- 阶段5后端当前提供显式取消、最多两个无依赖工作 Agent 并行和费用预留；前端已经完成对应合同、代理和交互适配。恢复、等待用户输入、进程重启恢复和大型节点结果 Artifact 降级仍不存在，前端不能显示这些动作，也不能把有限事件回放描述为持续订阅。
...
| 阶段5：Agent Runtime 与工作流 | 后端已完成 `model_only_v1`、完整节点合同、持久事件、最多两个无依赖工作 Agent 并行、费用预留和显式取消；工具与恢复不属于当前配置 | 展示 Agent 分工、节点结果、Handoff、验证、审核和预算证据，并适配取消与并行组 | Run 下的 Agent 时间线和工作流详情 | 八个接口、取消动作、并行组和费用预留已接入；真实 Provider 端到端验收待完成，工具另等阶段4 | 进行中 |
...
阶段5后端已经完成 `model_only_v1`。当前前端已消费完整节点结果、持久化事件、显式取消、费用预留字段和最多两个工作 Agent 的执行组，不改变阶段5后端状态；本节剩余工作是使用真实 Provider 完成端到端验收。
...
- 当前 Agent Workflow 只显示阶段5合同中的已提交事实；工具执行提示仍只是能力预留，正式工具事件合同完成前不得显示伪造步骤或进度。
...
| 运行详情 | `/runs/[runId]` | 阶段1先展示 Task、Attempt、Retry、Artifact；阶段5当前增加 Agent Workflow 摘要、节点时间线和结果；其他阶段继续增量增加任务、工具、代码、MCP 和诊断证据 | 阶段1基线；阶段5只在内部功能门禁下启用当前八个接口 |
...
- 阶段5当前测试：八个接口合同、取消代理与跨用户拒绝、并行组、费用预留、SSE 解析、事件顺序/重复/缺口/有限回放、DeepSeek 结构化输出模式和生产构建已覆盖。真实 Provider 下的 JSON/SSE 创建、幂等回放、Run 冲突、11 类节点数据、审核策略、预算、失败/未知结果和刷新恢复继续作为端到端验证。
...
| 阶段5 | 已接入 `model_only_v1` 创建/发现/结果/节点/事件、Agent/Turn/Handoff、验证、审核、取消、并行组和费用预留 | 后端八个接口和完整节点合同；既有 BFF 归属保护，工具视图另等阶段4 | 真实 Provider JSON/SSE、幂等、失败/未知结果和刷新恢复测试通过后完成 | 进行中 |
